...
def build_traefik_dashboard() -> dict:
    ...
    panels = [
        stat_panel(1, "Request Rate", {"x": 0, "y": 0, "w": 6, "h": 4},
                   f"sum(rate(traefik_service_requests_total{{{svc}}}[5m]))", unit="reqps"),
        stat_panel(2, "Error Rate (4xx/5xx)", {"x": 6, "y": 0, "w": 6, "h": 4},
        ...
        ts_panel(8, "Bandwidth (Service)", {"x": 12, "y": 12, "w": 12, "h": 8}, [
        ...
        ts_panel(9, "Errors Over Time", {"x": 0, "y": 20, "w": 12, "h": 8},
                 [prom_target(f'sum by (code) (rate(traefik_service_requests_total{{{svc},code=~"4..|5.."}}[5m]))', "{{code}}")], unit="reqps", stack=True),
        pie_panel(10, "Status Code Distribution (1h)", {"x": 12, "y": 20, "w": 12, "h": 8},
                  f"sum by (code) (increase(traefik_service_requests_total{{{svc}}}[1h]))", legend="{{code}}"),
        ts_panel(11, "Entrypoint Request Rate", {"x": 0, "y": 28, "w": 12, "h": 8},
                 [prom_target(f"sum by (entrypoint) (rate(traefik_entrypoint_requests_total{{{ep}}}[5m]))", "{{entrypoint}}")], unit="reqps"),
        ts_panel(12, "Entrypoint P95 Latency", {"x": 12, "y": 28, "w": 12, "h": 8},
        ...
        ts_panel(13, "Request Rate by Method", {"x": 0, "y": 36, "w": 24, "h": 8},
